# ConjugateGradient（`hard/iterative_clustering/conjugate_gradient`）

| 项 | 内容 |
|----|------|
| 分类 | iterative_clustering |
| 难度 | hard |
| 实现模型 | 归约 + 迭代 |
| 数学定义 | `共轭梯度解 Ax=b` |
| 输入 / 输出 | 见 design.md（实现时定 dtype/shape） |
| 关键考点 | 见下 |
| 书写顺序 | 本级第 75 题 |
| 状态 | ⬜ |

## 关键考点（CUDA）

- 迭代内核：每轮 launch 的开销与收敛检查的组织
- KMeans：分配步（距离归约）与更新步（质心累加）交替
- KNN 的距离矩阵分块与 top-k 维护

## 说明

- 本目录为**算子骨架**：`README.md`(卡片) + `design.md`(设计模板)；实现时先填 design，再长出 `naive.cu` / `optimized.cu` / `library.md` / `bench.md`（工作流见[根 README](../../README.md)）。
- 验收：CPU golden 通过 + compute-sanitizer 通过；**完成以优化闭环为标志**（三版对比有数据）。

