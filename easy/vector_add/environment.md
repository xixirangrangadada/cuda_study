...
Kaggle 容器禁用性能计数器，无法用 ncu 定位瓶颈。后续可选：

1. **租带 root 的云 GPU**（AutoDL / vast.ai / 云厂商 GPU 实例）
   - 有 root → 可跑 ncu（必要时 `sudo ncu`）
   - 选实例时确认允许 profiler / 性能计数器
2. **本地自有显卡后本机跑**（实习攒钱购卡 / 已有卡）
   - 本机 NVIDIA 卡 + 管理员权限 → ncu 直接可用，无容器限制，最自由

**在此之前**：用「cudaEvent 计时 + 利用率 + A/B 对比」维持优化迭代；
拿到可 profile 的环境后，再用 ncu 的 `SpeedOfLight` 段确认瓶颈。
