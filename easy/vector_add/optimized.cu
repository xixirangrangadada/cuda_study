...
// float4向量化
__global__ void add_vecv4(const float4* __restrict__ a,const float4* __restrict__ b,float4* __restrict__ c,int n4){
	// 计算起点
...
__global__ void add_tail(const float* a,const float* b,float* c,int base,int n){
...
inline void CudaAdd(int n){
	...
	if(n4>0){
		...
		add_vecv4<<<blocks,threads>>>(
				reinterpret_cast<const float4*>(d_a),
				reinterpret_cast<const float4*>(d_b),
				reinterpret_cast<float4*>(d_c),n4);
	}
	if(remain){
		add_tail<<<1,remain>>>(d_a,d_b,d_c,n4 * 4, n);
	}
	...
	for(int i=0;i<n;i++){
		...
		if(fabsf(predictValue-h_c[i])>1e-2){
			failedCount++;	
			#ifdef DEBUG
